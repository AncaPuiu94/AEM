...
[
    ...
    /**
     * Flag that determines if we're currently switching from source edit mode to WYSIWYG
     * mode
     * @type Boolean
     * @private
     */
    togglingSourceEdit: false,
    ...
    // overriding CQ.Ext.form.Field#processPath
    ...
    onRender: function(ct, position) {
        ...
        iframe.src= CQ.Ext.isSecure ? (CQ.Ext.SSL_SECURE_URL || "javascript:;") : "javascript:;";
        ...
        if (parentDialog) {
            ...
            var beforeSubmitListener = function(e) {
                if (this.initialized) {
                    this.syncValue();
                }
            };
            this.listenersToRemove.push({"element" : parentDialog,
                        "event" : "beforesubmit", "listener" : beforeSubmitListener});
            parentDialog.on("beforesubmit", beforeSubmitListener, this);
        }
    },
    ...
    onDestroy: function() {
        ...
        while (this.listenersToRemove.length > 0) {
            var info = this.listenersToRemove.pop();
            info.element.un(info.event, info.listener, this);
        }
    },

    // overriding CQ.Ext.form.HtmlEditor#syncValue
    syncValue: function() {
        if (this.initialized && (!this.sourceEditMode || this.togglingSourceEdit)) {
